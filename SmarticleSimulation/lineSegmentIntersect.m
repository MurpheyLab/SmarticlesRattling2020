function out = lineSegmentIntersect(XY1,XY2,...
    parThresh,distThresh,lenMx) %#codegen
...
coOrient = sign(X2_X1 .* X4_X3 + Y2_Y1 .* Y4_Y3);
...
%find parallel:
PAR_B = coOrient.* ((abs(denominator) < parThresh) &... %near-parallel lines (based on cross-product)
    (abs((u_a-0.5).*lenMx'-coOrient.*(u_b-0.5).*lenMx)<0.75) &... %close longitudinally
    (min(abs(u_a-0.5),abs(u_b-0.5)).*abs(denominator)<distThresh)); %and transversely
